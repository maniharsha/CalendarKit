...
class SwipeLabelView: UIView {
    ...
    weak var state: DayViewState? {
        ...
        didSet {
            ...
            labels.first!.text = isDefaultScheduleScreen! ? dayArray[(state?.selectedDate.weekday)! - 1] : state?.selectedDate.format(with: .full)
        }
    }
    ...
    public init(isDefaultScheduleScreen:Bool) {
        super.init(frame: .zero)
...
extension SwipeLabelView: DayViewStateUpdating {
    func move(from oldDate: Date, to newDate: Date) {
        ...
        labels.last!.text = isDefaultScheduleScreen! ? dayArray[newDate.weekday - 1] : newDate.format(with: .full)
