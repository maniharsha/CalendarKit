...
public protocol DaySelectorItemProtocol: class {
    ...
    var isDefaultScheduleScreen: Bool {get set}
...
class DaySelector: UIView, ReusableView {
    ...
    public var isDefaultScheduleScreen:Bool = false
    ...
    init(startDate: Date = Date(), isDefaultScheduleScreen:Bool = false, daysInWeek: Int = 7) {
    ...
    func configure() {
        for (increment, label) in items.enumerated() {
            label.isDefaultScheduleScreen = self.isDefaultScheduleScreen
